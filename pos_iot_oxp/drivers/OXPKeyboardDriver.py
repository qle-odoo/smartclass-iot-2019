...
class OXPKeyboardDriver(Driver):
    ...
    def action(self, data):
        ...
        if data.get('action') == 'change_all_led':
            self._change_all_led_status()         
    ...
    def _change_all_led_status(self):
        led_status = 0 in self._input_device.leds()
        led_status1 = 1 in self._input_device.leds()
        led_status2 = 2 in self._input_device.leds()
        self._input_device.set_led(0, int(not led_status))
        self._input_device.set_led(1, int(not led_status1))
        self._input_device.set_led(2, int(not led_status2))